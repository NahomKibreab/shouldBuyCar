...
describe("#shouldBuyCar()", function () {
  ...
  it("should return false when the car is a hatchback and pink", function () {
    const car = { color: "pink", type: "hatchback" };
    const shouldBuy = shouldBuyCar(car);
    assert.isFalse(shouldBuy);
  });
  ...
  it("should return true when the car has 11 litres/100km and is under or equal to $5000", function () {
    const car = { litresPer100km: 11, price: 500 };
    const shouldBuy = shouldBuyCar(car);
    assert.isTrue(shouldBuy);
  });
